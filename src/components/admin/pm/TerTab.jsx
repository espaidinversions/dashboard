...
import { loadPMTerOverridesTableResult, upsertTerOverride } from "../../../db.js";
import { InlineInput } from "./InlineInput.jsx";

export function TerTab() {
  const { tc } = useTheme();
  ...
  const load = useCallback(async () => {
    setLoading(true);
    const { data, error } = await loadPMTerOverridesTableResult();
    if (error) toast({ message: "Error carregant TER: " + error.message, type: "error" });
    setRows(data);
    setLoading(false);
  }, [toast]);
