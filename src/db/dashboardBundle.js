...
export async function fetchRawDashboardRows() {
  ...
  const [cc, fm, pl, co, sr, pe] = await Promise.all([
    fetchAllCapitalCallRows(),
    supabase.from("fund_meta").select(FUND_META_SELECT_COLUMNS.join(",")),
    supabase.from("pipeline").select(PIPELINE_SELECT_COLUMNS.join(",")).order("id"),
    supabase.from("portfolio_companies").select(PORTFOLIO_COMPANY_SELECT_COLUMNS.join(",")).order("nom"),
    supabase.from("searchers").select(SEARCHER_SELECT_COLUMNS.join(",")).order("nom"),
