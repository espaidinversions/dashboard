-- 20260728130000_preserve_fund_meta_import_columns.sql
-- Make replace_dashboard_bundle preserve the import-managed fund_meta columns
-- (fi_end, geography, sector) across a snapshot republish.
--
-- Root cause: the bundle payload only carries (vehicle_id, fons, tvpi, irr), and
-- the function wipes fund_meta then re-inserts just those four columns. Any
-- column not in the payload — fi_end, and now geography/sector — was blanked on
-- every republish, forcing a manual backfill afterwards.
--
-- Fix: snapshot the import-managed columns into a temp table BEFORE the wipe,
-- then UPDATE them back onto the re-inserted rows by vehicle_id. The existing
-- DELETE->INSERT flow and its ordering are left intact (fund_meta must be cleared
-- before private_entities is deleted, otherwise the ON DELETE SET NULL FK would
-- null out fund_meta.vehicle_id and violate its PK — this is also why a plain
-- ON CONFLICT upsert cannot replace the wipe here).
...
BEGIN
  ...
  -- Carry import-managed fund_meta columns (fi_end/geography/sector) across the
  -- wipe. Snapshot BEFORE deleting; ON COMMIT DROP cleans it at end of the RPC's
  -- transaction. Referenced pg_temp-qualified because SET search_path = public.
  ...
  IF p_fund_meta_rows IS NOT NULL AND COALESCE(jsonb_array_length(p_fund_meta_rows), 0) > 0 THEN
    ...
    -- Restore the import-managed columns onto the freshly re-inserted rows.
    UPDATE fund_meta fm
       SET fi_end    = c.fi_end,
