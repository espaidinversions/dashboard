-- Add fund_meta.strategy (jsonb weight map: fund commitment split across
-- strategy/fund-type buckets: Small/Mid/Large Buyout, Growth, VC, Turnaround,
-- Real Estate & Infraestructure). Sourced from the Matrius sheet of the
-- Allocation Excel (per-strategy row totals), same methodology as geography/sector.
-- Feeds the "by Fund Type" dimension of the Asset Allocation view.

ALTER TABLE fund_meta ADD COLUMN IF NOT EXISTS strategy JSONB;

-- Recreate the republish RPC so `strategy` (like geography/sector) survives a
-- dashboard bundle wipe. Only the fund_meta carry block changes vs the prior
-- definition; the rest is copied verbatim to keep the function whole.
...
BEGIN
  ...
  -- Carry import-managed fund_meta columns (fi_end/geography/sector/strategy) across the wipe.
  IF p_fund_meta_rows IS NOT NULL THEN
    CREATE TEMP TABLE _fund_meta_carry ON COMMIT DROP AS
      SELECT vehicle_id, fi_end, geography, sector, strategy FROM fund_meta;
