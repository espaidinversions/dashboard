-- Add fund_meta.allocation (jsonb weight map: the vehicle's fund-type
-- allocation split — Fons de Fons / Fons Primari / Fons Secundari /
-- Fons de Coinversió — as manually curated on the fund one-pager's
-- Classificació card). Unlike geography/sector/strategy (sourced from the
-- Matrius Excel), allocation is user-entered via the dashboard UI. When NULL,
-- the one-pager falls back to the transaction-derived underlying mix.
--
-- Like the other import/UI-managed columns (fi_end/geography/sector/strategy),
-- allocation must survive a dashboard bundle republish, so it is added to the
-- carry block of replace_dashboard_bundle below.

ALTER TABLE fund_meta ADD COLUMN IF NOT EXISTS allocation JSONB;

-- Recreate the republish RPC so `allocation` (like geography/sector/strategy)
-- survives a dashboard bundle wipe. Only the fund_meta carry block changes vs
-- the prior definition (20260728140000); the rest is copied verbatim to keep
-- the function whole.
...
BEGIN
  ...
  -- Carry import/UI-managed fund_meta columns (fi_end/geography/sector/strategy/allocation)
  -- across the wipe. Snapshot BEFORE deleting; ON COMMIT DROP cleans it at end of
  -- the RPC's transaction. Referenced pg_temp-qualified because SET search_path = public.
  IF p_fund_meta_rows IS NOT NULL THEN
    CREATE TEMP TABLE _fund_meta_carry ON COMMIT DROP AS
      SELECT vehicle_id, fi_end, geography, sector, strategy, allocation FROM fund_meta;
